                ...
                <div class="col-md-6">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Creer une nouvelle categorie</h3>
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="slug" name="slug" value="{{old('slug')}}" disabled class="form-control " placeholder="this will be generated automatically">
                            </div>
                            <div class="form-group">
                                <label for="category_id">Niveau de la categorie</label>
                                <select name="category_id" id="category_id" class="form-control">
                                    <option value="0" {{old('category_id') === 0 ? 'selected' : ''}}>Categorie Principale</option>
                                    <optgroup label="Sous categorie de">
                                        @foreach($categories as $c)
                                            <option value="{{$c->id}}" {{old('category_id') === $c->id ? 'selected' : ''}} title="{{$c->description}}">
                                                {{$c->name}}
                                            </option>
                                        @endforeach
                                    </optgroup>
                            ...
                            <div class="form-group">
                                <div>
                                    <label for="pic">Image de la categorie</label>
                            ...
                            <div class="form-group">
                                <label for="featured">Categorie en vedette ? </label>
                                <input type="checkbox" name="featured" id="featured" {{old('featured') ? 'checked' : ''}} value="on"  data-bootstrap-switch>                                @error('description')
                                <div class="invalid-feedback">{{$message}}</div>
                                @enderror
                            </div>
                        </div>
